---
title: "AWS desde 0 (I): Cómputo, almacenamiento y contenedores"
...
AWS ofrece decenas de servicios, y uno de los primeros retos al empezar es entender qué hace cada uno y cuándo usarlo. En este post se cubren los servicios fundamentales de cómputo y almacenamiento: EC2 para servidores virtuales, S3 para almacenamiento de objetos, EBS y EFS para almacenamiento adjunto a instancias, Glacier para archivado a largo plazo, y EKS para ejecutar contenedores. El objetivo es tener una visión práctica de cada servicio antes de profundizar en cada uno por separado.

## Contenido

1. [Amazon EC2](#introducción-a-amazon-ec2)
2. [Amazon S3](#introducción-a-amazon-s3)
3. [Amazon EBS](#almacenamiento-en-bloques-amazon-elastic-block-store-ebs)
4. [Amazon EFS](#almacenamiento-de-archivos-amazon-elastic-file-system-efs)
5. [Amazon S3 Glacier](#almacenamiento-de-largo-plazo-amazon-s3-glacier)
6. [Amazon EKS](#kubernetes-en-aws-introducción-a-amazon-eks)
...
EKS no es un servicio de almacenamiento en sí, pero se apoya directamente en EBS y EFS para gestionar el almacenamiento persistente de sus pods. Por eso tiene sentido tratarlo aquí, una vez que ya conocemos los bloques de almacenamiento sobre los que trabaja.
