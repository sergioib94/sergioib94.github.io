...
## Troubleshooting: permisos rotos en el volumen de Grafana tras la migración

Unos días después de dar por cerrada la migración, empecé a notar que tanto Ubuntu como el propio Windows anfitrión se ralentizaban de forma notable al trabajar con el lab. `docker ps -a` reveló la causa inmediata:

| CONTAIRED ID | IMAGE | STATUS |
|---|---|---|
...
Con solo un 2% de uso en la partición de WSL2, el espacio en disco quedaba descartado. El siguiente paso fue mirar directamente el contenido del volumen:

```bash
docker volume inspect monitoring_grafana-storage
sudo ls -la /var/lib/docker/volumes/monitoring_grafana-storage/_data
...
```bash
sudo chown -R 472:472 /var/lib/docker/volumes/monitoring_grafana-storage/_data
docker compose -f ~/noc-soc/monitoring/docker-compose.yml restart grafana
docker ps -a --filter "name=monitoring-grafana-1"
```
